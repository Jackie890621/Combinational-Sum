...
int main(int argc, const char * argv[])
{
    ...
    while(!fin.eof()) {
        ...
        if(check(fcheck, sol)) correct_num++;
        else cout << "Wrong case: target=" << target << " limit=" << negative_limit << " number of candidates=" << candidates_num << endl;
    ...
    end_time = clock();
    cout << "Total pass : "<< 100.0*correct_num/(case_num) << " percentages of cases" <<endl;
    cout << "Exc time: " << (end_time-start_time)/CLOCKS_PER_SEC << " secs" << endl;
